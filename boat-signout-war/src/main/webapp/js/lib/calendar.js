...
//	getHolidays.cache = {};
	...
	Calendar.prototype._calculate_hour_minutes = function(data) {
		...
		$.each(data.events, function(k, e) {
			var s = moment(new Date(parseInt(e.start)));
			var f =  moment(new Date(parseInt(e.end)));

			e.start_hour = s.format("h:mm a");
			e.end_hour = f.format("h:mm a");

			if(s.format("YYYY-MM-DD") != f.format("YYYY-MM-DD")) {
				e.start_hour = s.format("YYYY-MM-DD h:mm a");
				e.end_hour = f.format("YYYY-MM-DD h:mm a");
			}
		...
		switch($.type(source)) {
			...
			case 'string':
				if(source.length) {
					loader = function() {
						var events = [];
						var params = {from: self.options.position.start.getTime(), to: self.options.position.end.getTime(), boat: self.options.boat};
...
//			self.view(view);
			var dt = $('[data-cal-date]', this).data('cal-date');
			location.href = 'dayview.html?dt=' + dt;
		});
	...
	Calendar.prototype._update_month_year = function() {
		...
		$('.cal-month-day, .cal-year-box .span3')
			...
			.on('click', function(event) {
				if(self.selectedCell && self.selectedCell != $(this)){
					self.selectedCell.css("background-color", self.oldBackground);
				}
				if(self.selectedCell != $(this)){
					self.selectedCell = $(this);
					self.oldBackground = self.selectedCell.css("background-color");
					self.selectedCell.css("background-color","#c1c1c1");
				}
				
				self.selectedDate = $(this).children('[data-cal-date]').data('cal-date');
	...
	function showEventsList(event, that, slider, self) {
		...
		var dt = $('[data-cal-date]', cell).data('cal-date')
		...
		slider.slideUp('fast', function() {
			...
			var data = {};
			data.events = self.getEventsBetween(parseInt(event_list.data('cal-start')), parseInt(event_list.data('cal-end')));
